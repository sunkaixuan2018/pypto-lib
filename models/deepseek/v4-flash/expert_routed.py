...
def expert_routed(
    ...
    for local_e in pl.parallel(N_LOCAL_EXPERTS):
        ...
        for tt in pl.parallel(e_tiles):
            ...
            # Keep each W2 accumulator tile inside one mixed Cube+Vector task:
            # matmul -> dequant/route-weight -> BF16.  This avoids materializing
            # the full INT32 W2 output in GM and launching a second epilogue task.
            ...
            with pl.spmd(
                ...
                for dg in pl.range(W2_INNER):
                    ...
                    w2_scale_chunk = routed_w2_scale[local_e : local_e + 1, d0 : d0 + D_OUT_TILE]
