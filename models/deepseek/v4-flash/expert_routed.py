...
def expert_routed(
    ...
    # Each local expert owns a disjoint recv_y row slab. Submit W13 for one
    # expert wave before submitting that wave's downstream chain. This keeps
    # W2 from waiting behind every local W13 task without making W2 compete
    # with all remaining experts as in full per-expert interleaving.
    for wave_base in pl.range(0, N_LOCAL_EXPERTS, EXPERT_WAVE):
        for wave_i in pl.parallel(EXPERT_WAVE):
            local_i = wave_base + wave_i
        ...
        for wave_e in pl.parallel(EXPERT_WAVE):
            local_e = wave_base + wave_e
            # This expert's row slab of the two accumulators.
                ...
                with pl.spmd(
                    ...
                    for ag in pl.pipeline(ACT_GATE_INNER, stage=2):
                        ...
                        recv_x_scale_dq = pl.reshape(
                            recv_scale_dq[local_e : local_e + 1, tt0 : tt0 + RECV_TILE],
                            [RECV_TILE, 1],
                        )
