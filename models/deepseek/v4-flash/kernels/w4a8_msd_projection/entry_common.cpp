...
constexpr uint32_t kOriginalM = 16;
constexpr uint32_t kM = 2 * kOriginalM;
...
// Physical input is packed [N,K], equivalent to logical [K,N] column-major.
using BType = Gemm::GemmType<ElementI4, layout::ColumnMajor>;
...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t *args) {
    ...
    // Signature order:
    //   0=acc_planes[2M,N], 1=packed_planes[2M,K/2],
    //   2=packed_weight[N,K/2].
    ...
    AscendC::GlobalTensor<ElementI4> activation;
    activation.SetGlobalBuffer(
        reinterpret_cast<__gm__ ElementI4 *>(
            tensor_data<int8_t>(args, 1)));
    AscendC::GlobalTensor<ElementI4> weight;
    ...
    for (uint32_t inner = 0; inner < kInner; ++inner) {
        ...
        // int4b_t indexing is in logical nibble elements, so the column-major
        // offset is n0*K even though the backing Tensor is byte-packed.
