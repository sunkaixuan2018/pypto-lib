...
using BType = Gemm::GemmType<ElementI4, layout::zN>;
...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t *args) {
    ...
    // Signature: 0=acc_planes[M,N], 1=packed_planes[M,K/2],
    // 2=packed_weight_zN[K*N/2].
    ...
    const auto layout_b =
        layout::zN::MakeLayout<ElementI4>(kK, kN);
    const layout::RowMajor layout_c{kM, kN, kN};
    const GemmCoord task_shape{kM, kNTile, kK};
    const auto task_b_layout =
        layout_b.GetTileLayout(MatrixCoord{kK, kNTile});
    ...
    for (uint32_t inner = 0; inner < kInner; ++inner) {
        ...
        const int64_t b_offset =
            layout_b.GetOffset(MatrixCoord{0, n0});
        auto task_b = weight[b_offset];
