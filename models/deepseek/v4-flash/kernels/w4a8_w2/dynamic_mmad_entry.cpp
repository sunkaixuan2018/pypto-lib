...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t *args) {
    // Signature order:
    //   0=output, 1=activation, 2=expanded weight_kn, 3=workspace.
    ...
    flush_row(row);
    {
        typename MatmulKernel::Params params{
            ...
            LayoutC{kM, kN},
            {},
            reinterpret_cast<GM_ADDR>(tensor_data<int8_t>(args, 3)),
        };
        MatmulKernel kernel;
...

    {
        Arch::Resource<ArchTag> resource;
        {
            ...
            if (active) {
                ...
                AscendC::GlobalTensor<int8_t> workspace;
                workspace.SetGlobalBuffer(tensor_data<int8_t>(args, 3));
                auto ub =
                    resource.ubBuf.template GetBufferByByte<int8_t>(0);

                AscendC::DataCopyExtParams input_copy(
                    256, 256, kN - kNTile, 0, 0);
                ...
                for (uint32_t tile = 0; tile < kTileCount; ++tile) {
                    ...
                    const uint64_t source_offset =
                        (static_cast<uint64_t>(tile) * kKTile +
                         lane * 256) *
                            kN +
                        block_idx * kNTile;
                    AscendC::DataCopyPad(
                        ub, weight[source_offset], input_copy, input_pad);
                    ...
                    const uint64_t workspace_offset =
                        (static_cast<uint64_t>(block_idx) * kStages +
                         stage) *
                            kKTile * kNTile +
                        lane * 256 * kNTile;
                    AscendC::DataCopyPad(
                        workspace[workspace_offset], ub, output_copy);
                    row[7] += 1;
