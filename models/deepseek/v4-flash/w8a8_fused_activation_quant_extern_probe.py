...
# -----------------------------------------------------------------------------------------------------------
"""Validate the CANN W8A8 activation and row-quant phase as an AIV-only extern."""
...
_ENTRY = _KERNEL_DIR / "vector_entry.cpp"
_VLLM_ASCEND = Path(
    os.environ.get(
        "VLLM_ASCEND_SOURCE",
...
def fused_activation_quant_cce(
    ...
    weight_dummy: pl.Tensor,
    weight_scale: pl.Tensor,
) -> pl.Tensor: ...


@pl.jit
def fused_activation_quant_probe(
    workspace: pl.Tensor[[M, N], pl.INT32],
    x_scale: pl.Tensor[[M], pl.FP32],
    group_list: pl.Tensor[[EXPERTS], pl.INT64],
    weight_scale: pl.Tensor[[EXPERTS, N], pl.FP32],
    x_dummy: pl.Tensor[[1], pl.INT8],
    weight_dummy: pl.Tensor[[1], pl.INT8],
    out: pl.Out[pl.Tensor[[M, OUT_N], pl.INT8]],
    out_scale: pl.InOut[pl.Tensor[[M], pl.FP32]],
):
    with pl.spmd(
        VECTOR_BLOCKS,
        name_hint="w8a8_fused_activation_quant_extern",
        sync_start=True,
    ):
        out = fused_activation_quant_cce(
            out,
            out_scale,
            workspace,
            x_dummy,
...
def build_specs():
    ...
    return [
        ...
        TensorSpec(
            ...
            [1],
            torch.int8,
            init_value=lambda: torch.zeros(1, dtype=torch.int8),
        ),
        TensorSpec(
            "weight_dummy",
            [1],
...
if __name__ == "__main__":
    ...
    result = run_jit(
        fn=fused_activation_quant_probe,
        specs=build_specs(),
        golden_fn=golden,
        compile_cfg=dict(),
        runtime_cfg=dict(
